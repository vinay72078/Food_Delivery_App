...
function Receipt({ order }) {
  ...
  const addressParts = [delivery.address, delivery.city, delivery.pincode].filter(Boolean);
  ...
  return (
    <div className="receipt">
      ...
      <div className="receipt-section">
        ...
        <div className="receipt-row"><span>Date</span><strong>{formatDate(order.createdAt)}</strong></div>
        <div className="receipt-row"><span>Status</span><strong>{STATUS_LABELS[order.status] || order.status}</strong></div>
      </div>

      <div className="receipt-section">
        <div className="receipt-section-title">Customer details</div>
        <div className="receipt-row"><span>Name</span><strong>{customerName}</strong></div>
        <div className="receipt-row"><span>Phone</span><strong>{customerPhone}</strong></div>
        <div className="receipt-row"><span>Email</span><strong>{customerEmail}</strong></div>
      </div>

      <div className="receipt-section">
        <div className="receipt-section-title">Delivery address</div>
        <div className="receipt-address">{fullAddress}</div>
      </div>
      ...
      <div className="receipt-footer">Thank you for ordering with Foodie!</div>
    </div>
  );
}
